...
func Test_requestLogger(t *testing.T) {
	...
	if logBuffer.String() != expectedLogString {
		t.Errorf("log string did return expected string. expected string: %s. actual string: %s", expectedLogString, logBuffer.String())
	}

	if rr.Code != expectedStatusCode {
		t.Errorf("request status did return expected status. expected status: %d. actual status: %d", expectedStatusCode, rr.Code)
	}
}
